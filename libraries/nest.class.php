...
class Nest {
    ...
    function __construct($username=null, $password=null) {
        ...
        $this->username = env('NEST_USER', 'username');
        $this->password = env('NEST_PASS', 'production');

        $this->cookie_file = '../cache/nest_php_cookies_' . md5($username . $password);
        static::secure_touch($this->cookie_file);

        $this->cache_file = '../cache/nest_php_cache_' . md5($username . $password);
    ...
    private function doRequest($method, $url, $data_fields=null, $with_retry=TRUE) {
        ...
        $curl_cainfo = '../cache' . DIRECTORY_SEPARATOR . 'cacert.pem';
